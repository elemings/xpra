...
%if 0%{?with_webp}
CFLAGS="$RPM_OPT_FLAGS" %{__python2} setup.py build build_ext --enable-webp --enable-webpmux -I/usr/include/xpra -L/usr/lib64/xpra -R/usr/lib64/xpra
%else
...
%if 0%{?with_webp}
CFLAGS="$RPM_OPT_FLAGS" %{__python3} setup.py build build_ext --enable-webp --enable-webpmux -I/usr/include/xpra -L/usr/lib64/xpra -R/usr/lib64/xpra
%else
...
%changelog
* Tue Feb  4 2020 Eric Lemings <eric.lemings@ngc.com> - 5.4.1-2
- change build dependency from libwebp-devel to libwebp1-devel
- build with WEBP and WEBPMUX support
